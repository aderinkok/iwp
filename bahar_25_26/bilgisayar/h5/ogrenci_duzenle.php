...
if ($_POST) {
    ...
    if ($bolum != -1) {
        ...
        $stmt = $nesne->prepare($sql);
        $stmt->execute(['isim'=> $ad, 'soyisim'=> $soyad, 'bolum'=> $bolum,'id' => $kimlik]);
        $mesaj = "Güncelleme başarılı!";
        $bilgi = true;
        header("Location: ogrenci_duzenle.php?id=" . $kimlik."&mesaj=".urlencode($mesaj)."&bilgi=".urlencode($bilgi));
        }catch(PDOException $e){
            $mesaj = "Güncellemehatası: " . $e->getMessage();
            $bilgi = true;
        }
